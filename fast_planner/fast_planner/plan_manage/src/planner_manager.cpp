...
// 这个函数用以执行hybird A*以及后端b样条轨迹优化
// Eigen::Vector3d是一个C++类，用于表示三维向量。
bool FastPlannerManager::kinodynamicReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
  ...
  t1 = ros::Time::now();
  // kino_path_finder是指向KinodynamicAstar类的指针
  // 重置搜索路径算法的状态
...
bool FastPlannerManager::planGlobalTraj(const Eigen::Vector3d& start_pos) {
  ...
  const double            dist_thresh = 4.0; // 插入中间点的距离阈值

  // 遍历航路点，检查并插入中间点
  for (int i = 0; i < points.size() - 1; ++i) {
    inter_points.push_back(points.at(i));
    double dist = (points.at(i + 1) - points.at(i)).norm(); // 计算相邻点的距离

    // 如果距离超过阈值，则计算需要插入多少个中间点，并且进行插入
    if (dist > dist_thresh) {
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  if (!collide) {
    ...
    // 如果只检测到起始碰撞且没有碰撞结束，则认为初始轨迹已经处于障碍物内，不进行重新规划
